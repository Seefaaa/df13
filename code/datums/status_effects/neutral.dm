...
/datum/status_effect/offering/on_creation(mob/living/new_owner, obj/item/offer, mob/target_mob, give_alert_override)
	. = ..()
	if(!.)
		return
	offered_item = offer
	...
	if(!owner.CanReach(target) || IS_DEAD_OR_INCAP(target) || !target.can_hold_items())
		qdel(src)
		return

	register_candidate(target)

	if(!target) // no one around
		qdel(src)
		return
...
/datum/status_effect/offering/proc/check_owner_in_range(mob/living/carbon/source)
	...
	if(!istype(target) || !owner.CanReach(target) || IS_DEAD_OR_INCAP(target))
		remove_candidate(target)
