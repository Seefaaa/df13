	...
	switch(M.a_intent)
		...
		if("harm")
			if(HAS_TRAIT(M, TRAIT_PACIFISM))
				...
				return
			if(prob(M.get_skill_modifier(/datum/skill/combat/martial, SKILL_MISS_MODIFIER)))
			...
			var/damage = rand(M.dna.species.punchdamagelow, M.dna.species.punchdamagehigh) + M.get_skill_modifier(/datum/skill/combat/martial, SKILL_DAMAGE_MODIFIER)
			attack_threshold_check(damage, attack_type = BLUNT)
			log_combat(M, src, "attacked")
			updatehealth()
			if(stat != DEAD)
				M.adjust_experience(/datum/skill/combat/martial, 5)
			return TRUE
