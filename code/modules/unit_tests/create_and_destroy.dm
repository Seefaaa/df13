...
/datum/unit_test/create_and_destroy/Run()
	...
	var/list/ignore = list(
		...
		//Singleton
		/mob/dview,
		//This expects a button
		/atom/movable/screen/cooldown_overlay,
		//needs a mag type
		/obj/item/gun/ballistic/,
		/obj/item/gun/magic,
		//base chair doesn't have an armrest icon
		/obj/structure/chair,
	)
	...
	//Expects z level passed in
	ignore += typesof(/obj/effect/landmark/map_generator)
	// Expects missing icon states for overlays
	ignore += typesof(/obj/structure/closet/secure_closet)
	...
	for(var/type_path in typesof(/atom/movable, /turf) - ignore) //No areas please
		if(ispath(type_path, /turf))
			...
			spawn_at.ChangeTurf(/turf/open/floor/rock, /turf/baseturf_skipover)
