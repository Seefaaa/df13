...
/// Do some logic here before we run everything
/datum/round_event/ghost_role/proc/pre_start()
	return TRUE
...
/datum/round_event/ghost_role/proc/try_spawning(sanity = 0, retry = 0)
	...
	if(status == MAP_ERROR)
		...
		message_admins("[role_name] cannot be spawned due to lack of players \
			signing up.")
		deadchat_broadcast(" couldn't find [minimum_required] possible candidates.", "<b>[role_name]</b>", message_type=DEADCHAT_ANNOUNCEMENT)
...
/datum/round_event/ghost_role/proc/get_candidates(jobban, gametypecheck, be_special)
	...
	if(priority_candidates.len < minimum_required)
		regular_candidates = poll_ghost_candidates("Do you wish to play as \"[role_name]\"?", jobban, be_special)
	else
